...
- **Logging is non-negotiable.** Every F.R.M. action should be traceable — what changed, why, and what it affects — not just reflected as an updated doc with no reasoning trail. *(Gap: C.E.R. does not currently have a dedicated numbered Change Log — see "Open governance proposals" below.)*
...
- **Change-log Creation** — Claude writes change/decision log entries directly as part of doing the work, not as a separate step Wesley has to request. *(Gap: see Open governance proposals — there's no dedicated Change Log doc yet to write these into.)*
